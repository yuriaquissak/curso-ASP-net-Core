...
export class DataService {
  ...
  public getAllUsers(token) {
    return this.http.get(`${this.baseUrl}/v1/todos/users`, { headers: this.composeHeaders(token) });
  }

  public getTomorrowTodos(token) {
    return this.http.get(`${this.baseUrl}/v1/todos/undone/tomorow`, { headers: this.composeHeaders(token) });
  }

  public getAllTodos(token) {
    return this.http.get(`${this.baseUrl}/v1/todos`, { headers: this.composeHeaders(token) });
  }
  public getTodoById(token, id) {
    return this.http.get(`${this.baseUrl}/v1/todos/todo/${id}`,  { headers: this.composeHeaders(token) });
  }
  ...
  public updateTodo(data, token, id) {
    return this.http.put(`${this.baseUrl}/v1/todos/update/${id}`, data, { headers: this.composeHeaders(token) });
  }
